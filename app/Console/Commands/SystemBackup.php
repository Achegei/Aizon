...
class SystemBackup extends Command
{
    ...
    protected $description = 'Backup database and storage/public files, keep for 30 days';
    ...
    public function handle()
    {
        try {
            ...
            // ------------------------
            // STORAGE/PUBLIC BACKUP
            // ------------------------
            $storageDir = storage_path('app/public');
            $storageFile = "{$backupPath}/storage_{$date}.tar.gz";

            exec("tar -czf {$storageFile} -C {$storageDir} . 2>&1", $tarOutput, $tarResult);

            if ($tarResult !== 0) {
                throw new \Exception('Storage backup failed: ' . implode("\n", $tarOutput));
            }

            $this->info('Storage/public backup completed.');
