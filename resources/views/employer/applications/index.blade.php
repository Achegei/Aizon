        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach($applications as $application)
                <div class="bg-white rounded-xl shadow p-4 hover:shadow-lg transition duration-200">
                    ...
                    <p class="mt-2 text-gray-700">{{ Str::limit($application->cover_letter ?? 'No cover letter provided', 100) }}</p>
                    ...
                    <div class="mt-4 flex justify-between items-center">
                        ...
                        <span class="text-sm text-gray-400">{{ $application->status ?? 'Pending' }}</span>
                    </div>
                </div>
            @endforeach
        </div>
