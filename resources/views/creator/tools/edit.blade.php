...
<div class="container mx-auto p-6">
    <h1 class="text-2xl font-bold mb-6">Edit Tool: {{ $tool->title }}</h1>
    ...
    <form action="{{ route('creator.tools.update', $tool->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <input type="text" name="title" value="{{ old('title', $tool->title) }}" class="w-full border px-3 py-2 rounded" required>
        </div>

        <!-- Description -->
        <div class="mb-4">
            <label class="block mb-1 font-semibold">Description</label>
            <textarea name="description" class="w-full border px-3 py-2 rounded" rows="4">{{ old('description', $tool->description) }}</textarea>
        </div>

        <!-- Price -->
        <div class="mb-4">
            <label class="block mb-1 font-semibold">Price (USD)</label>
            <input type="number" step="0.01" name="price" value="{{ old('price', $tool->price) }}" class="w-full border px-3 py-2 rounded" required>
        </div>
        ...
        <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded">Update Tool</button>
    </form>
</div>
@endsection
