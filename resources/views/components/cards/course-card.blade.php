...
<div x-data="{ open: false }" class="bg-white rounded-2xl shadow-md hover:shadow-2xl transition duration-300 ease-in-out border border-gray-200 flex flex-col justify-between p-6">
    ...
    {{-- Course Info --}}
    <div>
        <img src="{{ $course->thumbnail_url ?? 'https://via.placeholder.com/400x200' }}" alt="{{ $course->title }}" class="rounded-lg mb-4">
        <h3 class="text-2xl font-bold text-gray-900">{{ $course->title }}</h3>
        <p class="text-gray-500 mt-1 text-sm">By {{ $course->creator->name ?? 'Unknown' }}</p>
        <p class="text-gray-400 mt-2 text-sm">{{ Str::limit($course->description, 120) }}</p>
        <p class="text-gray-400 mt-2 text-xs">🕒 Created {{ $course->created_at->diffForHumans() }}</p>
    </div>
    ...
    <div x-show="open" x-cloak 
        ...
        <div class="w-full pointer-events-auto flex justify-end">
            <div @click.away="open = false"
                ...
                {{-- Modal Content --}}
                <h2 class="text-3xl font-bold mb-4 text-gray-900">Enroll in {{ $course->title }}</h2>
                <p class="text-gray-600 mb-6 whitespace-pre-line">{{ $course->description }}</p>
